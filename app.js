...
function sshPayload() {
  ...
  if (protocol !== "SSH") {
    throw new Error("Use the matching VLESS, VMESS, TROJAN, or OVPN section to save that protocol.");
  }
...
function trojanPayload() {
  ...
  if (protocol !== "TROJAN") {
    throw new Error("Use the matching SSH, VLESS, VMESS, or OVPN section to save that protocol.");
  }
...
function renderAllServerLists() {
  ...
  renderServerList("trojanList", "trojanSearch", (server) => isTrojanProtocol(server.protocol) || server.protocol === "Shadowsocks");
}
