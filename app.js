...
function profileNetworkOf(profile) {
  return profile.country_network || "";
}
...
function renderProfileDropdowns() {
  ["v2ray", "ssh", "ovpn", "trojan", "ss"].forEach((prefix) => {
    ...
    state.profiles.filter(profileIsActive).forEach((profile) => {
      ...
      option.textContent = `${profileNameOf(profile)} — ${profileNetworkOf(profile) || "No network"}`;
...
function renderProfiles() {
  ...
  const filtered = state.profiles.filter((profile) => {
    return `${profileNameOf(profile)} ${profileNetworkOf(profile)} ${profileIdOf(profile)}`.toLowerCase().includes(term);
  });
    ...
    <article class="item-card">
      <div class="item-card-header">
        ...
        <div>
          ...
          <p class="item-subtitle">${escapeHtml(profileIdOf(profile) || profile.id)} ${profileNetworkOf(profile) ? `- ${escapeHtml(profileNetworkOf(profile))}` : ""}</p>
        </div>
...
function profilePayload() {
  const profileId = safeId($("profilePublicId").value) || safeId($("profileCustomId").value);
...
async function saveProfile(event) {
  ...
  try {
    ...
    if (existingId) delete data.createdAt;
    const customId = safeId($("profileCustomId").value) || data.profileId;
    const targetRef = existingId
