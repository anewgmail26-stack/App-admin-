...
function baseServerData(prefix, protocol) {
  ...
  return {
    server_name: $(`${prefix}Name`).value.trim(),
    country: $(`${prefix}Country`).value.trim(),
    flag: $(`${prefix}Flag`).value.trim(),
    protocol,
    ...
    sort_order: numberValue($(`${prefix}SortOrder`).value),
    uuid: "",
    alter_id: 0,
    security: "",
    network_type: "",
...
function renderProfileDropdowns() {
  ["v2ray", "ssh", "ovpn", "trojan", "ss"].forEach((prefix) => {
    ...
    select.innerHTML = '<option value="">No linked profile</option>';

    state.profiles.forEach((profile) => {
      const option = document.createElement("option");
      option.value = profile.id;
      option.dataset.profileName = profile.profile_name || "";
      option.textContent = `${profile.profile_name || "Unnamed Profile"} — ${profile.country_network || "No network"}`;
...
function v2rayPayload() {
  ...
  validateRequired(data, ["server_name", "protocol", "host", "port", "uuid", "network_type", "status"], "V2Ray requires Server Name, Protocol, Host/IP, Port, UUID, Network Type, Status, and Sort Order.");
  if ($("v2raySortOrder").value === "") throw new Error("V2Ray requires Sort Order.");
  return data;
}

function sshPayload() {
  const data = baseServerData("ssh", "SSH");
...
function ovpnPayload() {
  const data = baseServerData("ovpn", "OVPN");
  data.username = $("ovpnUsername").value.trim();
  data.password = $("ovpnPassword").value.trim();
  data.ovpn_config = $("ovpnConfig").value.trim();
  if (!data.server_name || (!data.host && !data.ovpn_config) || !data.status || $("ovpnSortOrder").value === "") {
    throw new Error("OpenVPN requires Server Name, Host/IP or OpenVPN Config, Status, and Sort Order.");
  }
  return data;
}

function trojanPayload() {
  const data = baseServerData("trojan", "Trojan");
...
function ssPayload() {
  const data = baseServerData("ss", "Shadowsocks");
...
function renderAllServerLists() {
  renderServerList("v2rayList", "v2raySearch", (server) => ["V2Ray VMess", "V2Ray VLESS"].includes(server.protocol));
  renderServerList("sshList", "sshSearch", (server) => server.protocol === "SSH");
  renderServerList("ovpnList", "ovpnSearch", (server) => server.protocol === "OVPN" || server.protocol === "OpenVPN");
  renderServerList("trojanList", "trojanSearch", (server) => server.protocol === "Trojan" || server.protocol === "Shadowsocks");
}
    ...
    <article class="item-card">
      <div class="item-card-header">
        <div>
          ...
          <p class="item-subtitle">${escapeHtml(server.host || "No host")}:${escapeHtml(server.port || "")}</p>
        </div>
...
function linkedProfileLabel(server) {
  if (!server.profile_id && !server.profile_name) return "None";
  const profile = state.profiles.find((item) => item.id === server.profile_id);
  if (profile) return profile.profile_name || "Unnamed Profile";
  if (server.profile_id) return "Profile missing";
  return server.profile_name || "None";
}

function sensitiveBadge(server) {
  if (server.protocol === "V2Ray VMess" || server.protocol === "V2Ray VLESS") {
    return `
  ...
  if (server.protocol === "Trojan") return `<span class="badge">Password ${escapeHtml(maskSecret(server.trojan_password))}</span>`;
  if (server.protocol === "Shadowsocks") return `<span class="badge">${escapeHtml(server.ss_method || "method")}</span>`;
...
function setCommon(prefix, server) {
  ...
  $(`${prefix}Name`).value = server.server_name || "";
  $(`${prefix}Country`).value = server.country || "";
  $(`${prefix}Flag`).value = server.flag || "";
  $(`${prefix}Host`).value = server.host || "";
  $(`${prefix}Port`).value = server.port || "";
  $(`${prefix}Status`).value = server.status || "inactive";
...
function editServer(id) {
  ...
  if (server.protocol === "V2Ray VMess" || server.protocol === "V2Ray VLESS") {
    ...
    $("v2rayProtocol").value = server.protocol || "V2Ray VMess";
    ...
    $("v2rayPublicKey").value = server.public_key || "";
    $("v2rayShortId").value = server.short_id || "";
    $("v2raySpiderX").value = server.spider_x || "";
  ...
  } else if (server.protocol === "OVPN" || server.protocol === "OpenVPN") {
    setCommon("ovpn", server);
    $("ovpnUsername").value = server.username || "";
    $("ovpnPassword").value = server.password || "";
    $("ovpnConfig").value = server.ovpn_config || "";
    $("ovpnFormTitle").textContent = "Edit OpenVPN Server";
  } else if (server.protocol === "Trojan") {
    setCommon("trojan", server);
    $("trojanPassword").value = server.trojan_password || "";
...
function resetV2rayForm() {
  ...
  $("v2rayProtocol").value = "V2Ray VMess";
...
function importV2rayLink() {
  ...
  try {
    ...
    if (link.startsWith("vless://")) {
      ...
      resetV2rayForm();
      $("v2rayProtocol").value = "V2Ray VLESS";
...
function profilePayload() {
  ...
  return {
    ...
    dns_option: boolValue("profileDns"),
