...
function ovpnPayload() {
  ...
  data.sslHost = $("ovpnSslHost").value.trim();
  data.sniHost = $("ovpnSniHost").value.trim();
  ...
  if (!isOvpnProtocol(protocol)) {
    throw new Error("Use the matching SSH, VLESS, or TROJAN section to save that protocol.");
  }
  ...
  if (isOvpnProtocol(protocol)) {
    ...
    if (OVPN_MODES_WITH_SSL.includes(ovpnMode) && !data.sniHost && !data.sslHost) {
      throw new Error("SNI/SSL host is required");
    }
  }
...
function renderAllServerLists() {
  renderServerList("v2rayList", "v2raySearch", (server) => ["V2Ray VMess", "V2Ray VLESS", "VLESS"].includes(server.protocol));
  ...
  renderServerList("trojanList", "trojanSearch", (server) => server.protocol === "Trojan" || server.protocol === "TROJAN" || server.protocol === "Shadowsocks");
}
...
function sensitiveBadge(server) {
  ...
  if (server.protocol === "V2Ray VMess" || server.protocol === "V2Ray VLESS" || server.protocol === "VLESS") {
    return `
  ...
  if (server.protocol === "Trojan" || server.protocol === "TROJAN") return `<span class="badge">Password ${escapeHtml(maskSecret(server.trojan_password))}</span>`;
...
function editServer(id) {
  ...
  if (server.protocol === "V2Ray VMess" || server.protocol === "V2Ray VLESS" || server.protocol === "VLESS") {
    ...
    $("ovpnSslHost").value = server.sslHost || "";
    $("ovpnSniHost").value = server.sniHost || "";
  ...
  } else if (server.protocol === "Trojan" || server.protocol === "TROJAN") {
