...
Every server form includes a `Linked Profile / Pack` dropdown. The dropdown is loaded from the `profiles` collection and displays each option as:

```text
profile_name — country_network
```
...
```js
{
  profile_id: "profile_document_id",
  profile_name: "Profile Name"
}
```

Leaving the dropdown empty saves both fields as empty strings. If a linked profile is deleted later, the admin panel keeps working and shows `Profile missing` on affected server cards.

The Android app can use `profile_id` later to group or filter servers by pack. `profile_name` is also saved as a convenient display fallback, but `profile_id` is the better stable key for matching a server to a profile document.
