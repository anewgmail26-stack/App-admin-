...
Supported server protocols are `SSH`, `VLESS`, `TROJAN`, `OVPN`, `OpenVPN`, `Imported OpenVPN`, `V2Ray VMess`, `V2Ray VLESS`, `Trojan`, and `Shadowsocks`. Protocol-specific fields are optional in Firestore and are saved as empty strings, `0`, or `false` when unused.
...
The OpenVPN section can parse pasted `.ovpn` config text. It saves the full text in `ovpnConfigText` and also mirrors it to `ovpn_config` for backward compatibility. It tries to extract the first `remote host port` line for manual review.

OpenVPN supports these modes:
...
For `OVPN`, at least one of `ovpnConfigText` or `ovpnConfigUrl` is required. GitHub `blob` URLs are rejected because the app needs a direct raw `.ovpn` link. Payload modes require `payload`; `PAYLOAD_PROXY` also requires `proxyHost` and `proxyPort`; SSL modes require `sniHost` or `sslHost`. Password and proxy password fields are not shown in server cards and are left blank in edit mode unless the admin enters a replacement.
